import { MONTHS, REGEX, SEPARATOR } from "@/constants";
...
export const covertAmountToNumber = (amount: string): number =>
  Number(amount.replace(/,/g, ""));

export const convertFromStringToObject = (
  date: string,
): Record<string, string | number> => {
  const dateInstance = new Date(date);

  return {
    month: MONTHS[dateInstance.getMonth()].option,
    day: dateInstance.getDate(),
    year: dateInstance.getFullYear(),
  };
};

export const convertFromStringToInteger = (id: string) => parseInt(id);
