...
export const OrderStatus = ({ status, period }: OrderStatusProps) => {
  switch (status) {
    ...
    case 0:
      return (
        <>
          <Text className="dark:text-dark-romance font-light opacity-100 text-tertiary leading-[21px] w-full">
            Order was delivered {period} days ago.
          </Text>
          <Text className="p-2 mt-4 font-bold text-white dark:text-white bg-killarney text-xs rounded-tremor-small leading-[10.5px] tracking-[0.18px] uppercase">
            delivered
          </Text>
        </>
      );
