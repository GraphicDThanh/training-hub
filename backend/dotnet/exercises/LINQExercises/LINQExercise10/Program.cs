/*
10. Write a program in C# Sharp to accept list members through the keyboard and display them more than a specific value.
Test Data :
Input the number of members on the List : 5
Member 0 : 10
Member 1 : 48
Member 2 : 52
Member 3 : 94
Member 4 : 63
Input the value above you want to display the members of the List : 59
Expected Output :
The numbers greater than 59 are :
94
63
*/
Console.WriteLine("Input members on the List :");
string numbersString = Console.ReadLine();

Console.WriteLine("Input the value above you want to display the members of the List :");
string numberGte = Console.ReadLine();

// convert number list string to list int
var numbers = numbersString.Split(" ").ToList().Select(n => int.Parse(n));

// convert number gte than
var gteNumber = int.Parse(numberGte);


var query = numbers.Where(n => n > gteNumber);

foreach (var n in query)
{
    Console.WriteLine(n);
}