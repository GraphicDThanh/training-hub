...
class StockTest(unittest.TestCase):
    ...
    # skip test on win
    @unittest.skipIf(sys.platform.startswith("win"), "skip run on windows")
    ...
    def test_stock_price_should_give_the_lastest_price(self):
        self.goog.update(datetime(2014, 2, 10), price=10)
...
class StockTrendTest(unittest.TestCase):
    ...
    #     self.given_a_series_of_prices(self.goog, [8, 10, 12])
    #     self.assertTrue(self.goog.is_increasing_trend())


    # def test_increasing_trending_is_false_if_price_decreases(self):
    #     self.given_a_series_of_prices(self.goog, [8, 10, 0])
    #     self.assertFalse(self.goog.is_increasing_trend())


    # def test_increasing_trend_is_false_if_price_equal(self):
    #     self.given_a_series_of_prices(self.goog, [8, 10, 10])
    #     self.assertFalse(self.goog.is_increasing_trend())
...
class StockCrossOverSignalTest(unittest.TestCase):
    ...
    # Copy from code example, quite not clear enough
...
## 1. Add suite function in test file(tests/test_stock.py) and write script(run_test_stock_custom.py) to run it
def suite():
   loader = unittest.TestLoader()
   test_suite = unittest.TestSuite()
   # add single test
   test_suite.addTest(StockTest('test_stock_update'))
   # add multi test use loaders
