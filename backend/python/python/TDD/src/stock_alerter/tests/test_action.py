...
class EmailActionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.action = EmailAction(to="thanh.nguyen@asnet.com.vn")


    def test_email_is_sent_to_the_righ_server(self):
    ...
    def test_connection_closed_after_sending_mail(self):
        self.action.execute("MFST has crossed $10 price level")
    ...
    def test_email_is_sent_with_the_right_subject(self):
        self.action.execute("MSFT has crosses $10 price level")
    ...
    def test_email_is_sent_when_action_is_executed(self):
        expected_message = {
            ...
            "Message": "MSFT has crosses $10 price level",
            "To": "thanh.nguyen@asnet.com.vn",
            "From": "alerts@stocks.com"
        }

        self.action.execute("MSFT has crosses $10 price level")
        self.mock_smtp.send_message.assert_called_with(MessageMatcher(expected_message))
...
class MessageMatcher:
    ...
    def __eq__(self, other):
        return self.expected["Subject"] == other["Subject"] and self.expected["From"] == other["From"] and self.expected["To"] == other["To"] and self.expected["Message"] == other._payload
