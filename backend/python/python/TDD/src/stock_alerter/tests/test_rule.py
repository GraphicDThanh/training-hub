...
class PriceRuleTest(unittest.TestCase):
    ...
    def test_a_PriceRule_is_False_if_the_condition_not_meet(self):
        rule = PriceRule('GOOG', lambda stock: stock.price < 10)
    ...
    # Pattern: using attribute example
    # Attribute "slow" use to filter out test in run_test.py
    test_a_PriceRule_only_depends_on_its_stock.slow = True
    test_a_PriceRule_only_depends_on_its_stock.integration = True
    test_a_PriceRule_only_depends_on_its_stock.python = ["2.6", "3.4"]
