...
db = SQLDatabase.from_uri(
    settings.supabase.DB_URL,
    sample_rows_in_table_info=0,  # no sample row to avoid hallucination
)
...
def check_and_execute_query_tool(query: str) -> str:
    """
    **IMPORTANT**:
    - Determine `Query Intent` value in your input. If it is `cart`, do not call this tool.
    Use this tool to validate and execute SQL queries.

    Requirements:
    - Prior calls to sql_db_list_tables and sql_db_schema.
    - Ensure SQL syntax correctness.

    Example query: "SELECT * FROM products WHERE name LIKE '%cotton%';"
    ...
    query_checker_chain = query_check_prompt | chat_model.default_model.bind_tools(
