...
Load documents (txt, json, csv) from data/knowledge_base folder, splits them into chunks,
...
from common.constants.llms import EMBEDDING_MODEL
from rag.loader import BaseLoader
    ...
    def process_single_file(
        ...
        try:
            print(
                ...
                utils.get_path(
                    file_name,
                    f"{settings.SERVER_FOLDER}/{settings.KNOWLEDGE_BASE_FOLDER}",
                ),
            )
            ...
            text_splitter: RecursiveCharacterTextSplitter = (
                RecursiveCharacterTextSplitter(
                    chunk_size=settings.splitter.TEXT_CHUNK_SIZE,
                    chunk_overlap=settings.splitter.TEXT_CHUNK_OVERLAP,
                )
            )
...
def main() -> None:
    ...
    if not parser_args.force and utils.is_chroma_db_exist(settings.vector_store.PATH):
